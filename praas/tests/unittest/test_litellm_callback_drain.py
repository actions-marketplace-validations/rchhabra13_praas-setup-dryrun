...
from praas.config_loader import global_settings
...
def test_drain_still_flushes_after_a_task_timeout(clean_litellm_callbacks, monkeypatch):
    ...
    monkeypatch.setattr(
        "praas.algo.ai_handlers.litellm_helpers._get_global_logging_worker",
        lambda: _SpyWorker(),
    )
    monkeypatch.setattr(
        "praas.algo.ai_handlers.litellm_helpers._is_litellm_task", lambda task: True)
