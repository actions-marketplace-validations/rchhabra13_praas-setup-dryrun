...
class TestGitLabProvider:
    ...
    def gitlab_provider(self, mock_gitlab_client, mock_project):
        with patch('praas.git_providers.gitlab_provider.gitlab.Gitlab', return_value=mock_gitlab_client), \
             patch('praas.git_providers.gitlab_provider.get_settings') as mock_settings:
    ...
    def test_should_publish_review_as_thread_reflects_config(self, gitlab_provider, configured):
        with patch("praas.git_providers.gitlab_provider.get_settings",
    ...
    def test_should_publish_review_as_thread_defaults_false(self, gitlab_provider):
        ...
        with patch("praas.git_providers.gitlab_provider.get_settings", return_value=settings):
...
class TestGitLabGlobalSettings:
    ...
    def test_loads_group_praas_settings(self):
        ...
        with patch("praas.git_providers.gitlab_provider.get_settings") as ms:
            ms.return_value.config.use_global_settings_file = True
            result = provider._get_global_repo_settings()
        assert result == b"[pr_reviewer]\nnum_max_findings = 5\n"
        provider.gl.projects.get.assert_called_with("mygroup/praas-settings")
        proj.files.get.assert_called_once_with(file_path=".praas.toml", ref="main")
...
class TestGitLabRelevantDiff:
    ...
    def test_fallback_is_logged_once_not_once_per_version(self):
        ...
        with patch("praas.git_providers.gitlab_provider.get_logger") as mock_logger:
