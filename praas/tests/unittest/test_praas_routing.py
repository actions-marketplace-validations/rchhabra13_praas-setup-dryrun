...
def _patch_request_dependencies(monkeypatch, validate_result=(True, None), update_settings_fn=None):
    ...
    monkeypatch.setattr(praas_module.CliArgs, "validate_user_args", lambda args: validate_result)
    monkeypatch.setattr(praas_module, "update_settings_from_args", update_settings_fn)
...
async def test_handle_request_routes_known_command_and_notifies(monkeypatch):
    ...
    monkeypatch.setitem(praas_module.command2class, "custom", FakeTool)

    handled = await praas_module.Praas(ai_handler="fake-ai")._handle_request(
...
async def test_handle_request_rejects_forbidden_cli_args(monkeypatch):
    ...
    handled = await praas_module.Praas()._handle_request("https://example/pr/1", "/custom --secret=value")
...
async def test_handle_request_wrapper_returns_false_on_exception(monkeypatch):
    ...
    monkeypatch.setattr(praas_module.Praas, "_handle_request", raise_error)

    handled = await praas_module.Praas().handle_request("https://example/pr/1", "/review")
...
async def test_handle_request_answer_uses_reviewer_answer_mode_and_notifies(monkeypatch):
    ...
    _patch_request_dependencies(monkeypatch)
    monkeypatch.setattr(praas_module, "PRReviewer", FakeReviewer)
...
async def test_handle_request_answer_preserves_quoted_question_as_single_arg(monkeypatch):
    ...
    handled = await praas_module.Praas()._handle_request(
...
async def test_handle_request_returns_false_for_unknown_command(monkeypatch):
    ...
    handled = await praas_module.Praas()._handle_request("https://example/pr/1", "/unknown")
