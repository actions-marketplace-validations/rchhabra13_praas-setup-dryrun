...
class TestApplyRepoSettings:
    ...
    them — preventing cross-repo `.praas.toml` state leaks reported in #2345.
    """

    def test_repo_settings_from_toml_are_applied(self, fresh_global_settings, monkeypatch):
        monkeypatch.setattr(
            "praas.git_providers.utils.get_git_provider_with_context",
    ...
    def test_repo_without_toml_does_not_inherit_previous_repo_settings(
        ...
        # Request 1: Repo A with .praas.toml — mutates only this request's settings clone.
        monkeypatch.setattr(
            "praas.git_providers.utils.get_git_provider_with_context",
            lambda url: FakeGitProvider(REPO_A_TOML),
        )
        ...
        # Request 2: Repo B with no .praas.toml — fresh clone of the unmutated global_settings.
        monkeypatch.setattr(
            "praas.git_providers.utils.get_git_provider_with_context",
    ...
    def test_unknown_section_does_not_leak_to_next_repo(self, fresh_global_settings, monkeypatch):
        """Catches the case where a repo's `.praas.toml` introduces a section
