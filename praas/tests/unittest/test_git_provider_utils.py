...
from praas.git_providers import utils
from praas.git_providers.git_provider import GitProvider
from praas.git_providers.utils import apply_repo_settings, handle_configurations_errors


def test_clone_does_not_suppress_interrupts(tmp_path):
    def interrupt(*args, **kwargs):
        raise KeyboardInterrupt("clone interrupted")

    provider = SimpleNamespace(
        _prepare_clone_url_with_token=lambda url: url,
        _clone_inner=interrupt,
    )

    with pytest.raises(KeyboardInterrupt, match="clone interrupted"):
        GitProvider.clone(provider, "https://example.invalid/repository.git", str(tmp_path), remove_dest_folder=False)
...
def test_apply_repo_settings_attributes_global_error_and_still_applies_local(monkeypatch):
    ...
    try:
        ...
        assert "global `praas-settings` settings repository" in provider.comments[0]
...
def test_handle_configurations_errors_uses_persistent_comment_when_supported():
    ...
    assert comment["initial_header"] == "❌ **praas failed to apply 'local' repo settings**"
    assert comment["update_header"] is False
    assert comment["final_update_message"] is False
    assert "praas failed to apply 'local' repo settings" in comment["body"]
...
def test_handle_configurations_errors_keeps_markdown_details_when_persistent_comment_is_missing():
    ...
    assert len(provider.comments) == 1
    assert "praas failed to apply 'local' repo settings" in provider.comments[0]
...
def test_handle_configurations_errors_publishes_each_error():
    ...
    assert "global `praas-settings` settings repository" in provider.comments[1]
...
def test_get_cached_global_settings_skips_oversized_values(monkeypatch):
    from praas.git_providers import git_provider as gp
