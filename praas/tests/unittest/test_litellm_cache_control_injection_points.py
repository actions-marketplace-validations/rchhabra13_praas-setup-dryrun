...
Covers the config-boundary handling that Praas flagged: native TOML arrays, JSON-string
...
async def test_native_toml_list_is_passed_through(monkeypatch):
    ...
    with patch("praas.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock) as mock_call:
