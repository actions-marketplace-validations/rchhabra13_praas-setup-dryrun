...
from praas.algo.skills_loader import (Skill, _parse_skill_file,
                                         discover_skills,
...
class TestResourceGathering:
    ...
    def _apply_repo_skills_toml(self, monkeypatch, repo_toml: bytes):
        from praas.config_loader import get_settings
        from praas.git_providers import utils as gp_utils
    ...
    def test_repo_settings_cannot_override_skills_paths(self, monkeypatch):
        """A malicious repo's .praas.toml must not be able to set skills.paths —
    ...
    def test_repo_settings_can_override_safe_skills_keys(self, monkeypatch):
        ...
        repo's .praas.toml; only the host-only skills.paths is refused.
        """
