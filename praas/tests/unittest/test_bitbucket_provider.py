...
from praas.algo.types import EDIT_TYPE, FilePatchInfo
from praas.git_providers import BitbucketServerProvider
from praas.git_providers.bitbucket_provider import BitbucketProvider
...
class TestBitbucketServerProvider:
    ...
    def test_get_repo_settings_missing_file_not_logged_as_error(self):
        # A missing .praas.toml is expected/optional and must not be logged as an
        ...
        with patch("praas.git_providers.bitbucket_server_provider.get_logger") as mock_get_logger:
...
def _clear_global_settings_cache():
    from praas.git_providers import git_provider as _gp
...
class TestBitbucketGlobalSettings:
    ...
    def test_loads_workspace_praas_settings(self):
        ...
        with patch("praas.git_providers.bitbucket_provider.requests.request",
                   side_effect=[repo_resp, file_resp]) as rq, \
             patch("praas.git_providers.bitbucket_provider.get_settings") as ms:
        ...
        assert "myws/praas-settings" in rq.call_args_list[0].args[1]
        assert "src/main/.praas.toml" in rq.call_args_list[1].args[1]
    ...
    def test_no_access_403_returns_empty_and_caches(self):
        ...
        with patch("praas.git_providers.bitbucket_provider.requests.request", return_value=repo_resp) as rq, \
             patch("praas.git_providers.bitbucket_provider.get_settings") as ms:
            ms.return_value.config.use_global_settings_file = True
    ...
    def test_disabled_returns_empty(self):
        provider = self._provider()
        with patch("praas.git_providers.bitbucket_provider.requests.request") as rq, \
             patch("praas.git_providers.bitbucket_provider.get_settings") as ms:
...
class TestBitbucketLocalSettingsRobustness:
    def test_get_repo_settings_ignores_error_response_for_local(self):
        ...
        with patch("praas.git_providers.bitbucket_provider.requests.request", return_value=resp), \
             patch("praas.git_providers.bitbucket_provider.get_settings") as ms:
            ms.return_value.config.use_global_settings_file = False
            result = provider.get_repo_settings()
        assert result == ""
