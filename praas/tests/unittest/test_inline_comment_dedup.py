...
from praas.algo import inline_comment_dedup as d
from praas.git_providers.github_provider import GithubProvider
from praas.git_providers.gitlab_provider import GitLabProvider
...
def test_build_markers():
    assert d.build_markers("aaaaaaaaaaaa", None) == "<!-- praas-dedup: aaaaaaaaaaaa -->"
    out = d.build_markers("aaaaaaaaaaaa", "bbbbbbbbbbbb")
    assert "<!-- praas-dedup: aaaaaaaaaaaa -->" in out
    assert "<!-- praas-dedup-code: bbbbbbbbbbbb -->" in out
...
def _patch_flag(value):
    gs = patch("praas.git_providers.github_provider.get_settings")
    m = gs.start()
...
def test_github_filters_seen_and_marks_new():
    ...
    assert "<!-- praas-dedup:" in published[0]["body"]
...
def test_github_flag_off_publishes_unmarked():
    ...
    published = p.pr.create_review.call_args.kwargs["comments"]
    assert len(published) == 1
    assert "praas-dedup" not in published[0]["body"]
...
def test_gitlab_posts_new_with_marker_and_skips_duplicate():
    p = _gl_provider([])
    gs = patch("praas.git_providers.gitlab_provider.get_settings")
    m = gs.start()
    ...
    try:
        ...
        assert "<!-- praas-dedup:" in first["body"]
...
def test_gitlab_flag_off_posts_unmarked():
    ...
    try:
        ...
        assert "praas-dedup" not in body
    finally:
        gs.stop()
...
def test_github_does_not_send_pre_transform_fingerprint_to_api():
    ...
    assert "<!-- praas-dedup-code:" in published[0]["body"]
...
def test_gitlab_fallback_note_carries_marker_and_records():
    ...
    try:
        ...
        assert "<!-- praas-dedup:" in note_body
...
def test_gitlab_deletion_anchored_on_source_line():
    ...
    try:
        ...
        assert f"<!-- praas-dedup: {expected} -->" in first["body"]
    finally:
        gs.stop()
...
def test_has_marker_requires_wellformed_marker():
    assert d.has_marker("body\n\n<!-- praas-dedup: a1b2c3d4e5f6 -->")
    assert not d.has_marker("a comment that merely mentions <!-- praas-dedup: in prose")
