...
def _provider(local_settings=None, global_settings=None):
    ...
    provider.repo_obj = FakeRepo({".praas.toml": local_settings} if local_settings is not None else {})
    provider.github_client = FakeGithubClient(
        {"org/praas-settings": FakeRepo({".praas.toml": global_settings})}
...
def test_get_repo_settings_missing_local_file_logged_quietly(monkeypatch):
    # A missing local .praas.toml (404) is expected for most repos; it must be logged at debug,
    # not warning.
    from unittest.mock import patch

    provider = _provider()  # no local .praas.toml -> get_contents raises GithubException(404)
    settings = get_settings()
    original = settings.config.use_global_settings_file
    ...
    monkeypatch.delenv("PRAAS_CONFIG_BRANCH", raising=False)
    settings.set("CONFIG.CONFIG_BRANCH", "")  # ensure the default-branch load path runs
    try:
        with patch("praas.git_providers.github_provider.get_logger") as mock_get_logger:
...
def test_get_global_repo_settings_missing_repo_logged_quietly():
    # A missing/inaccessible <owner>/praas-settings repo is an expected fallback, so it must be
...
def test_get_repo_settings_config_branch_non_404_error_propagates(monkeypatch):
    ...
    monkeypatch.setenv("PRAAS_CONFIG_BRANCH", "release")
    settings = get_settings()
    original = settings.config.use_global_settings_file
    settings.config.use_global_settings_file = False
    try:
