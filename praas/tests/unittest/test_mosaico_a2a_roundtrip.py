...
Note: import praas.config_loader first to avoid the praas.log <->
custom_merge_loader circular import (mirrors server.py)."""
import os

import praas.config_loader  # noqa: F401  (import-order load; see module docstring)
...
class TestA2ARoundTripStubbedLLM:
    ...
    async def test_warmup_health_and_card(self, monkeypatch):
        ...
        from praas.mosaico.server import build_app
        app = build_app()
        async with _build_client(app) as client:
            ...
            body = card.json()
            assert body["name"] == "praas Solution Agent"
    ...
    async def test_supplied_diff_completed_with_artifact(self, monkeypatch):
        ...
        assert "Praas AI Review Report" in text
