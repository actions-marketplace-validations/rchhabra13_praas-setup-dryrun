...
import praas.algo.ai_handlers.litellm_ai_handler as litellm_handler
...
def mock_logger():
    ...
    with patch('praas.algo.ai_handlers.litellm_ai_handler.get_logger') as mock_log:
...
class TestLiteLLMReasoningEffort:
    ...
    async def test_gpt5_valid_reasoning_effort_none(self, monkeypatch, mock_logger):
        ...
        with patch('praas.algo.ai_handlers.litellm_ai_handler.acompletion', new_callable=AsyncMock) as mock_completion:
    ...
    async def test_gpt5_with_openai_prefix_triggers_reasoning_effort(self, monkeypatch, mock_logger):
        ...
        for model in prefixed_models:
            with patch(
                'praas.algo.ai_handlers.litellm_ai_handler.acompletion',
