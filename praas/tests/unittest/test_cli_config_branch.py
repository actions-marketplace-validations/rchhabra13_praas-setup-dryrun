...
def test_run_sets_config_branch_from_cli_flag():
    ...
    with patch("praas.cli.get_settings", return_value=fake_settings), patch(
        "praas.cli.Praas",
...
def test_run_sets_config_branch_from_env_var():
    ...
    with patch.dict("os.environ", {"PRAAS_CONFIG_BRANCH": "env-branch"}, clear=False), patch(
        "praas.cli.get_settings",
...
def test_run_reconciles_config_branch_when_absent():
    ...
    with patch.dict("os.environ", {}, clear=False), patch(
        ...
        _os.environ.pop("PRAAS_CONFIG_BRANCH", None)
        cli.run(inargs=["--pr_url=https://github.com/a/b/pull/1", "review"])
