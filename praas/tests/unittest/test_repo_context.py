...
from praas.algo import repo_context
from praas.algo.repo_context import (
    TRUNCATION_MARKER,
    build_repo_context,
...
def test_default_config_ships_agents_md_as_repo_context():
    ...
    config_path = Path(praas.__file__).parent / "settings" / "configuration.toml"
...
def test_build_repo_context_refreshes_process_cache_after_ttl(repo_context_settings):
    ...
    with patch("praas.algo.repo_context.time.monotonic", side_effect=[100, 100, 2000, 2000, 2000, 2000]):
        first_context = build_repo_context(first_provider)
        second_context = build_repo_context(second_provider)
...
def test_repo_context_cache_evicts_oldest_entry_when_full():
    ...
    with patch("praas.algo.repo_context.time.monotonic", return_value=100):
        cache["first"] = "one"
        cache["second"] = "two"
        cache["third"] = "three"
...
def test_provider_supports_repo_context_warns_once_for_unsupported_provider(repo_context_settings):
    ...
    with patch("praas.algo.repo_context.get_logger") as mock_get_logger:
