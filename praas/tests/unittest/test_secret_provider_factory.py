...
from praas.secret_providers import SUPPORTED_SECRET_PROVIDERS, get_secret_provider, validate_secret_provider_setting
...
class TestValidateSecretProviderSetting:
    ...
    def test_accepts_supported_providers(self, provider_id):
        with patch('praas.secret_providers.get_settings', return_value=self._settings(provider_id)):
            validate_secret_provider_setting()

    def test_accepts_unset_provider(self):
        with patch('praas.secret_providers.get_settings', return_value=self._settings("")):
            validate_secret_provider_setting()

    def test_rejects_unknown_provider(self):
        with patch('praas.secret_providers.get_settings', return_value=self._settings("gcs")):
    ...
    def test_does_not_construct_a_client(self):
        ...
        with patch('praas.secret_providers.get_settings', return_value=self._settings("aws_secrets_manager")):
            with patch('praas.secret_providers.get_secret_provider') as mock_build:
                validate_secret_provider_setting()
...
class TestSecretProviderFactory:
    ...
    def test_get_secret_provider_none_when_not_configured(self):
        with patch('praas.secret_providers.get_settings') as mock_get_settings:
            settings = MagicMock()
    ...
    def test_get_secret_provider_google_cloud_storage(self):
        with patch('praas.secret_providers.get_settings') as mock_get_settings:
            ...
            with patch('praas.secret_providers.google_cloud_storage_secret_provider.GoogleCloudStorageSecretProvider') as MockProvider:
    ...
    def test_get_secret_provider_aws_secrets_manager(self):
        with patch('praas.secret_providers.get_settings') as mock_get_settings:
            ...
            with patch('praas.secret_providers.aws_secrets_manager_provider.AWSSecretsManagerProvider') as MockProvider:
                mock_instance = MagicMock()
                MockProvider.return_value = mock_instance
