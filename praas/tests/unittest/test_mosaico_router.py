...
from praas.mosaico import dispatch
from praas.mosaico.dispatch import (_detect_verb, _diff_prose,
...
class TestPathPrUrl:
    ...
    async def test_pr_url_runs_handle_request_and_returns_artifact(self, monkeypatch, restore_settings):
        ...
        from praas.agent.praas import Praas
        monkeypatch.setattr(Praas, "handle_request", fake_handle_request)
        ...
        # _run_praas must inject the no-publish flags.
...
class TestPathSuppliedDiff:
    ...
    async def test_diff_with_question_mark_in_body_still_reviews(self, monkeypatch, restore_settings):
        ...
        from praas.agent.praas import Praas
        monkeypatch.setattr(Praas, "handle_request", fake_handle_request)
        monkeypatch.setattr("praas.tools.pr_questions.PRQuestions", _explode_prquestions)
...
class TestAskWithContext:
    ...
    async def test_pr_url_question_runs_prquestions(self, monkeypatch, restore_settings):
        ...
        praas_used = {"called": False}

        async def fail_handle_request(self, *a, **k):
            praas_used["called"] = True
            return True

        monkeypatch.setattr(dispatch, "_fetch_public_diff", fake_fetch_public_diff)
        monkeypatch.setattr("praas.tools.pr_questions.PRQuestions", FakePRQuestions)
        from praas.agent.praas import Praas
        monkeypatch.setattr(Praas, "handle_request", fail_handle_request)
        ...
        assert captured["pr_url"] == "mosaico://supplied-diff"
        assert global_settings.get("CONFIG.GIT_PROVIDER") == "mosaico_diff"
        assert praas_used["called"] is False
...
class TestPathFreeText:
    ...
    async def test_free_text_returns_guidance_not_internal_error(self, monkeypatch, restore_settings):
        """A context-free free-text ask must NOT call PRQuestions/Praas and must NOT
        ...
        assert out == "praas requires a PR URL or a supplied diff."
...
class TestDefensiveCapture:
    ...
    async def test_tool_exception_marks_failed(self, monkeypatch, restore_settings):
        ...
        monkeypatch.setattr(dispatch, "_fetch_public_diff", fake_fetch_public_diff)
        from praas.agent.praas import Praas
        monkeypatch.setattr(Praas, "_handle_request", raising_handle_request)
        _clear_artifact()
    ...
    async def test_legacy_callers_unaffected_when_flag_off(self, monkeypatch, restore_settings):
        ...
        import praas.algo.ai_handlers.litellm_ai_handler as litellm_mod
        import praas.mosaico.provider_registration  # noqa: F401
        from praas.agent.praas import Praas
        from praas.mosaico.diff_provider import parse_unified_diff
        ...
        ok = await Praas().handle_request("mosaico://supplied-diff",
    ...
    async def test_propagate_flag_does_not_outlive_a_contextless_run(self, monkeypatch, restore_settings):
        ...
        result = await dispatch._run_praas("mosaico://supplied-diff", "review")
    ...
    async def test_ask_that_raises_returns_error_fallback(self, monkeypatch, restore_settings):
        ...
        monkeypatch.setattr(dispatch, "_fetch_public_diff", fake_fetch_public_diff)
        monkeypatch.setattr("praas.tools.pr_questions.PRQuestions", RaisingPRQuestions)
...
class TestPublishOutputForced:
    """Prove that _run_praas and _run_ask force publish_output=False regardless
    ...
    @pytest.mark.asyncio
    async def test_run_praas_injects_no_publish_flags(self, monkeypatch, restore_settings):
        """_run_praas must pass --config.publish_output=false and
        ...
        from praas.agent.praas import Praas
        from praas.mosaico.dispatch import _run_praas
        monkeypatch.setattr(Praas, "handle_request", fake_handle_request)
        ...
        out = await _run_praas(PR_URL, "review")
    ...
    async def test_run_ask_forces_publish_output_false(self, monkeypatch, restore_settings):
        ...
        arg-injection trick used by _run_praas cannot apply; a settings.set() is
        required instead."""
        ...
        monkeypatch.setattr("praas.tools.pr_questions.PRQuestions", CapturingPRQuestions)

        from praas.mosaico.dispatch import _run_ask
