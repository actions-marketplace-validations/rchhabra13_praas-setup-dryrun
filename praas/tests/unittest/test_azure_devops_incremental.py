...
from praas.algo.utils import PRReviewHeader
from praas.git_providers import AzureDevopsProvider
from praas.git_providers.azuredevops_provider import (
    _AzureCommitAdapter,
    _to_naive_utc,
)
...
class TestGetIncrementalCommits:
    ...
    def test_populates_commits_range_and_files(self):
        ...
        prev = _comment(f"{PRReviewHeader.REGULAR.value}\nbody", review_time)
    ...
    def test_skips_merge_commits(self):
        ...
        provider.get_issue_comments = MagicMock(
            return_value=[_comment(PRReviewHeader.REGULAR.value, review_time)]
        )
    ...
    def test_all_commits_newer_than_review_falls_back_to_full(self):
        # Regression for Praas #7: when every commit is newer than the previous review there is
    ...
    def test_missing_date_commit_after_baseline_is_included(self):
        # Regression for Praas #1 (discussion r3518938542): a new commit whose author date is
    ...
    def test_incremental_resets_stale_diff_cache(self):
        # Regression for Praas #1: switching a reused provider into incremental mode must
