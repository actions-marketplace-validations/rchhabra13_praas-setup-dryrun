...
from praas.tools.pr_reviewer import PRReviewer, _last_yaml_key_for_subagent
...
async def test_prepare_prediction_requests_remaining_files_and_preserves_tuple_result():
    ...
    with patch(
        "praas.tools.pr_reviewer.get_pr_diff",
...
async def test_prepare_prediction_accepts_full_diff_string_when_token_budget_is_sufficient():
    ...
    with patch("praas.tools.pr_reviewer.get_pr_diff", return_value="diff"):
        await reviewer._prepare_prediction("model")
...
async def test_prepare_prediction_keeps_incremental_review_compatible_with_tuple_result():
    ...
    with patch("praas.tools.pr_reviewer.get_pr_diff", return_value=("diff", ["skipped.py"])):
        await reviewer._prepare_prediction("model")
...
def _render_review(reviewer, remaining_files, supports_gfm_markdown=False):
    ...
    with (
        patch("praas.tools.pr_reviewer.load_yaml", return_value={"review": {}}),
        patch("praas.tools.pr_reviewer.github_action_output"),
        patch("praas.tools.pr_reviewer.convert_to_markdown_v2", return_value="original review"),
    ):
        return reviewer._prepare_pr_review()


def test_prepare_pr_review_flags_key_issue_citing_a_file_outside_the_diff():
    """_prepare_pr_review must run key_issues_to_review through flag_ungrounded_issues before
    handing it to the renderer - a file the model cited that was never in the diff it was shown
    is a cheap, deterministic hallucination signal worth surfacing to the human reviewer."""
    ...
    reviewer.remaining_files_list = []
    reviewer.git_provider.get_diff_files.return_value = [
        FilePatchInfo(base_file="", head_file="", patch="", filename="src/real.py"),
    ]
    ...
    review_data = {
        "review": {
            "key_issues_to_review": [
                {"relevant_file": "src/real.py", "issue_content": "grounded finding"},
                {"relevant_file": "src/imaginary.py", "issue_content": "ungrounded finding"},
            ]
        }
    }

    with (
        patch("praas.tools.pr_reviewer.load_yaml", return_value=review_data),
        patch("praas.tools.pr_reviewer.github_action_output"),
        patch("praas.tools.pr_reviewer.convert_to_markdown_v2", return_value="rendered") as convert_mock,
    ):
...
def test_prepare_pr_review_places_coverage_footer_before_help_text():
    ...
    try:
        ...
        with patch("praas.tools.pr_reviewer.HelpMessage.get_review_usage_guide", return_value="help text"):
...
def test_set_review_labels_survives_missing_fields_under_multi_subagent_selection():
    """Under multi-subagent review, 'estimated_effort_to_review_[1-5]' and 'security_concerns'
    are only present when the sourcing subagent (correctness / security respectively) actually
    ran - e.g. a docs-only PR runs neither. Direct dict indexing on either would raise KeyError
    and, since both live in one try/except, silently drop the whole labels feature (including
    whichever label *did* have its data). Must degrade to "skip that one label", not crash."""
    settings = get_settings()
    original = {
        "publish_output": settings.config.publish_output,
        "require_estimate_effort_to_review": settings.pr_reviewer.require_estimate_effort_to_review,
    ...
    settings.pr_reviewer.enable_review_labels_security = True
    git_provider = MagicMock()
    git_provider.get_pr_labels.return_value = ["keep-me"]
    reviewer = _make_reviewer(git_provider)
    # A docs-only diff's multi-subagent review: only the docs subagent ran, so neither
    # correctness- nor security-sourced fields are present at all.
    data = {"review": {"key_issues_to_review": []}}

    try:
        reviewer.set_review_labels(data)  # must not raise
...
def test_set_review_labels_applies_effort_label_even_when_security_concerns_absent():
    """One field being absent (security_concerns) must not prevent the other (effort) from
    still producing its label - the two are independent, not all-or-nothing."""
    settings = get_settings()
    original = {
        "publish_output": settings.config.publish_output,
        "require_estimate_effort_to_review": settings.pr_reviewer.require_estimate_effort_to_review,
    ...
    reviewer = _make_reviewer(git_provider)
    # Only the correctness subagent ran (e.g. testing/security excluded this pass); effort is
    # present but security_concerns is not.
    data = {"review": {"estimated_effort_to_review_[1-5]": "2, easy"}}

    try:
        reviewer.set_review_labels(data)

        git_provider.publish_labels.assert_called_once_with(["Review effort 2/5"])
    finally:
        settings.config.publish_output = original["publish_output"]
        settings.pr_reviewer.require_estimate_effort_to_review = original["require_estimate_effort_to_review"]
        settings.pr_reviewer.require_security_review = original["require_security_review"]
...
def test_last_yaml_key_for_security_subagent_is_security_concerns():
    """security_concerns is emitted *after* key_issues_to_review in the security subagent's
    schema (pr_reviewer_prompts.toml). Using 'key_issues_to_review' as load_yaml's last_key
    for this subagent would make the YAML-repair fallback truncate the response before
    security_concerns - silently discarding the security subagent's actual finding."""
    assert _last_yaml_key_for_subagent("security") == "security_concerns"


def test_last_yaml_key_for_testing_and_docs_subagents_is_key_issues_to_review():
    assert _last_yaml_key_for_subagent("testing") == "key_issues_to_review"
    assert _last_yaml_key_for_subagent("docs") == "key_issues_to_review"


def test_last_yaml_key_for_correctness_subagent_respects_config_field_order():
    settings = get_settings()
    original = {
        "require_todo_scan": settings.pr_reviewer.get("require_todo_scan", False),
        "require_can_be_split_review": settings.pr_reviewer.get("require_can_be_split_review", False),
    }
    try:
        settings.pr_reviewer.require_todo_scan = False
        settings.pr_reviewer.require_can_be_split_review = False
        assert _last_yaml_key_for_subagent("correctness") == "key_issues_to_review"

        settings.pr_reviewer.require_todo_scan = True
        assert _last_yaml_key_for_subagent("correctness") == "todo_sections"

        # can_be_split is defined after todo_sections in the schema, so it wins when both
        # flags are on - it's genuinely the last field emitted, not todo_sections.
        settings.pr_reviewer.require_can_be_split_review = True
        assert _last_yaml_key_for_subagent("correctness") == "can_be_split"
    finally:
        settings.pr_reviewer.require_todo_scan = original["require_todo_scan"]
        settings.pr_reviewer.require_can_be_split_review = original["require_can_be_split_review"]


@pytest.mark.asyncio
async def test_multi_subagent_prediction_passes_correct_last_key_to_load_yaml_per_subagent():
    """Integration check: _get_multi_subagent_prediction must actually thread the per-subagent
    last_key through to load_yaml, not just have the helper function be correct in isolation."""
    reviewer = _make_reviewer()
    reviewer.git_provider.get_diff_files.return_value = []
    reviewer.patches_diff = ""

    async def fake_get_prediction(model, review_subagent=None):
        return f"canned response for {review_subagent}"

    reviewer._get_prediction = fake_get_prediction

    seen_last_keys = {}

    def fake_load_yaml(response_text, keys_fix_yaml=None, first_key="", last_key=""):
        subagent = response_text.rsplit(" ", 1)[-1]
        seen_last_keys[subagent] = last_key
        return {"review": {}}

    settings = get_settings()
    original_override = settings.pr_reviewer.get("subagents_override", [])
    settings.pr_reviewer.subagents_override = ["correctness", "security", "testing", "docs"]
    try:
        with patch("praas.tools.pr_reviewer.load_yaml", side_effect=fake_load_yaml):
            await reviewer._get_multi_subagent_prediction("gpt-x")
    finally:
        settings.pr_reviewer.subagents_override = original_override

    assert seen_last_keys["security"] == "security_concerns"
    assert seen_last_keys["correctness"] == "key_issues_to_review"
    assert seen_last_keys["testing"] == "key_issues_to_review"
    assert seen_last_keys["docs"] == "key_issues_to_review"
...
async def test_run_threads_only_the_final_review_comment(monkeypatch, persistent, thread_enabled):
    ...
    """
    from praas.tools import pr_reviewer as pr_reviewer_module
...
@pytest.mark.asyncio
async def test_run_removes_initial_comment_when_prediction_raises(monkeypatch):
    """A "Preparing review..." temp comment must not be left stuck on the PR forever: if every
    model/subagent fails inside retry_with_fallback_models (e.g. a rate-limited API key), the
    exception is caught by run()'s outer handler, which must still clean up the temp comment
    before returning/re-raising.
    """
    from praas.tools import pr_reviewer as pr_reviewer_module
    ...
    async def fake_retry_raises(prepare_fn, model_type=None):
        raise Exception("RESOURCE_EXHAUSTED: all subagent calls failed")

    monkeypatch.setattr(pr_reviewer_module, "extract_and_cache_pr_tickets", fake_extract_tickets)
    monkeypatch.setattr(pr_reviewer_module, "retry_with_fallback_models", fake_retry_raises)
...
@pytest.mark.asyncio
async def test_run_removes_initial_comment_when_prediction_raises_and_propagates(monkeypatch):
    """propagate_tool_errors=True must still clean up the temp comment before re-raising -
    cleanup happens regardless of whether the failure is ultimately swallowed or propagated."""
    from praas.tools import pr_reviewer as pr_reviewer_module
...
@pytest.mark.asyncio
async def test_run_removes_initial_comment_when_no_major_issues_to_publish(monkeypatch):
    """Same class of bug as the exception path: when the review completes but has nothing
    worth publishing (publish_output_no_suggestions=False and no major issues found), the temp
    comment must still be cleaned up rather than left dangling.
    """
    from praas.tools import pr_reviewer as pr_reviewer_module
