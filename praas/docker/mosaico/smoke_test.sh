...
# The-Praas/praas release workflow (Docker target `mosaico_agent`).
#
...
# (e.g. an unattended poll loop). Copy praas.env.example to .env and fill it in.
...
IMAGE="${IMAGE:-praas/praas:0.41.0-mosaico_agent}"
PORT="${PORT:-9000}"
CONTAINER="praas-mosaico-test"
...
assert c["name"] == "praas Solution Agent", c.get("name")
