...
from praas.algo.utils import update_settings_from_args
from praas.config_loader import get_settings, global_settings
from praas.git_providers.utils import apply_repo_settings
from praas.log import LoggingFormat, get_logger, setup_logger
from praas.secret_providers import (get_secret_provider,
...
async def handle_request(api_url: str, body: str, log_context: dict, sender_id: str, notify=None):
    ...
    with get_logger().contextualize(**log_context):
        await Praas().handle_request(api_url, body, notify)

async def _perform_commands_gitlab(commands_conf: str, agent: Praas, api_url: str,
...
def is_bot_user(data) -> bool:
    try:
        ...
        # the future. Normalize the value defensively: a misconfigured .praas.toml (string
...
async def gitlab_webhook(background_tasks: BackgroundTasks, request: Request):
    ...
    async def inner(data: dict):
        ...
        if request_token and secret_provider:
            ...
            # Timing-safe compare, matching the GitHub/Bitbucket Server/Gitea/Azure DevOps
            # webhook auth paths elsewhere in this codebase - a plain `==` here leaks timing
            # information proportional to the matching-prefix length.
            if not (isinstance(request_token, str) and isinstance(secret, str)
                    and hmac.compare_digest(request_token, secret)):
        ...
        if data.get('object_kind') == 'merge_request':
            ...
            if object_attributes.get('action') in ['open', 'reopen']:
                ...
                await _perform_commands_gitlab("pr_commands", Praas(), url, log_context, data)
            ...
            # for push event triggered merge requests
                ...
                await _perform_commands_gitlab("push_commands", Praas(), url, log_context, data)
