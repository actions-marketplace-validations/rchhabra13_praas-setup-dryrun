...
from praas.identity_providers import get_identity_provider
from praas.identity_providers.identity_provider import Eligibility
from praas.log import LoggingFormat, get_logger, setup_logger
from praas.servers.utils import DefaultDictWithTimeout, verify_signature
...
def handle_closed_pr(body, event, action, log_context):
    ...
    log_context["api_url"] = api_url
    get_logger().info("praas statistics for closed PR", analytics=True, pr_statistics=pr_statistics, **log_context)
...
async def _perform_auto_commands_github(commands_conf: str, agent: Praas, body: dict, api_url: str,
                                        log_context: dict):
    apply_repo_settings(api_url)
