...
from praas.git_providers import get_git_provider
from praas.git_providers.utils import apply_repo_settings
from praas.log import get_logger
from praas.servers.github_app import handle_line_comments
...
def _inject_artifact_context():
    ...
    artifact_path_env = (
        os.environ.get("ARTIFACT_PATH") or os.environ.get("PRAAS_ARTIFACT_PATH") or ""
    ).strip()
    artifact_instructions_env = (
        os.environ.get("ARTIFACT_INSTRUCTIONS") or os.environ.get("PRAAS_ARTIFACT_INSTRUCTIONS") or ""
    ).strip()
    ...
    try:
        from praas.algo.artifacts import load_artifact
...
async def run_action():
        ...
        if action == "synchronize":
            ...
            if is_true(push_trigger):
                ...
                for command in push_commands:
                    await Praas().handle_request(pr_url, command)
                return
    ...
    # Handle issue comment event
        ...
        if action in ["created", "edited"]:
            # Skip comments authored by bots (including praas's own
