...
from praas.algo import MAX_TOKENS
from praas.algo.git_patch_processing import extract_hunk_lines_from_patch
from praas.algo.run_details import get_run_details
from praas.algo.token_handler import TokenEncoder
from praas.algo.types import FilePatchInfo
from praas.config_loader import get_settings, global_settings
...
class PRReviewHeader(str, Enum):
    REGULAR = "## Praas AI Review Report"
    INCREMENTAL = "## Incremental Praas AI Review Report"
...
def _build_critical_findings_banner(review: dict, gfm_supported: bool = True) -> str:
    ...
    if not isinstance(key_issues, list):
        key_issues = []
    critical_issues = [
        issue for issue in key_issues
        if isinstance(issue, dict) and str(issue.get('severity', '')).strip().lower() == 'critical'
    ]

    security_concerns = review.get('security_concerns')
    has_security_flag = bool(security_concerns) and not is_value_no(security_concerns)

    if not critical_issues and not has_security_flag:
        return ""
...
def convert_to_markdown_v2(output_data: dict,
                           ...
                           git_provider=None,
                           files=None,
                           multi_subagent_mode: bool = False) -> str:
    """
    Convert a dictionary of data into markdown format.
    Args:
        output_data (dict): A dictionary containing data to be converted to markdown format.
        multi_subagent_mode (bool): Set when this review came from PRReviewer's opt-in multi-subagent
            fan-out (config.pr_reviewer.enable_multi_subagent_review) rather than a single holistic
            call. Purely additive: when False (the default, and always the case for the
            single-call path), rendering is byte-for-byte identical to before this flag existed.
            When True, a "Critical findings" banner is prepended above the table for any
            critical-severity issue or flagged security concern, so they can't be missed among
            ...
            if is_value_no(value):
                ...
                for i, issue in enumerate(issues):
                    try:
                        ...
                        # severity/category are only present in multi-subagent review's merged
                        # output; the single-call path's issues never carry these keys, so this
                        # leaves that path's rendering untouched.
                        issue_header_display = issue_header
                        severity = issue.get('severity')
                        if severity:
                            severity_badge = _SEVERITY_BADGE.get(str(severity).strip().lower(), str(severity).strip())
                            issue_header_display = f"{issue_header} {severity_badge}"
                        ...
                        if gfm_supported:
                            if reference_link is not None and len(reference_link) > 0:
                                if relevant_lines_str:
                                    issue_str = f"<details><summary><a href='{reference_link}'><strong>{issue_header_display}</strong></a>\n\n{issue_content}\n</summary>\n\n{relevant_lines_str}\n\n</details>"
                                else:
                                    issue_str = f"<a href='{reference_link}'><strong>{issue_header_display}</strong></a><br>{issue_content}"
                            else:
                                issue_str = f"<strong>{issue_header_display}</strong><br>{issue_content}"
                        else:
                            if reference_link is not None and len(reference_link) > 0:
                                issue_str = f"[**{issue_header_display}**]({reference_link})\n\n{issue_content}\n\n"
                            else:
                                issue_str = f"**{issue_header_display}**\n\n{issue_content}\n\n"
...
def get_max_tokens(model):
    ...
    logic:
    (1) If the model is in './praas/algo/__init__.py', use the value from there.
    ...
    if model in MAX_TOKENS:
        ...
        get_logger().error(f"Model {model} is not defined in MAX_TOKENS in ./praas/algo/__init__.py and no custom_model_max_tokens is set")
        raise Exception(f"Ensure {model} is defined in MAX_TOKENS in ./praas/algo/__init__.py or set a positive value for it in config.custom_model_max_tokens")
...
def show_relevant_configurations(relevant_section: str) -> str:
    ...
    markdown_text +="<br>These are the relevant [configurations](https://github.com/Praas/praas/blob/main/praas/settings/configuration.toml) for this tool:\n\n"
...
def get_version() -> str:
    ...
    try:
        return version('praas')
    except PackageNotFoundError:
        get_logger().warning("Unable to find package named 'praas'")
        return "unknown"
