...
BODY_MARKER_RE = re.compile(r"<!-- praas-dedup: ([a-f0-9]{12}) -->")
CODE_MARKER_RE = re.compile(r"<!-- praas-dedup-code: ([a-f0-9]{12}) -->")
...
def build_markers(body_fp: str, code_fp: Optional[str]) -> str:
    markers = [f"<!-- praas-dedup: {body_fp} -->"]
    if code_fp is not None:
        markers.append(f"<!-- praas-dedup-code: {code_fp} -->")
    return "\n".join(markers)
