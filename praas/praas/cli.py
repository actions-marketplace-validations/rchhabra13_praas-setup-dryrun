...
from praas.agent.praas import Praas, commands
from praas.algo.ai_handlers.litellm_helpers import (
    DEFAULT_CALLBACK_TIMEOUT_SECONDS, drain_litellm_callbacks,
    litellm_callbacks_registered)
from praas.algo.utils import get_version
from praas.config_loader import get_settings
from praas.log import get_logger, setup_logger
...
def set_parser():
    ...
    - ask_line - Ask a question about selected lines.

    - config / settings / help - Show configuration or command help.

    - similar_issue - Find related GitHub issues (requires an optional vector database).

    - add_docs - Suggest documentation additions.

    - generate_labels - Generate labels for the pull request.


    Configuration:
    To override a configuration parameter from 'configuration.toml', add --section.key=<value>.
    For example: 'python cli.py --pr_url=... review --pr_reviewer.extra_instructions="focus on the file: ..."'
    """)
    parser.add_argument('--version', action='version', version=f'praas {get_version()}')
    parser.add_argument('--pr_url', type=str, help='The URL of the PR to review', default=None)
    parser.add_argument('--issue_url', type=str, help='The URL of the Issue to review', default=None)
    parser.add_argument('--config-branch', type=str, help='Git branch to load .praas.toml from', default=None)
    parser.add_argument(
        "--extra_config_url",
        type=str,
        default=os.environ.get("PRAAS_EXTRA_CONFIG_URL"),
        help=(
            "URL or local path of an additional .praas.toml to merge before the "
            "repo-local config (e.g. shared/org defaults). Accepts http(s):// URLs or "
            "a filesystem path. For private endpoints, set PRAAS_EXTRA_CONFIG_AUTH_HEADER "
            "(e.g. 'PRIVATE-TOKEN: <token>' or 'JOB-TOKEN: $CI_JOB_TOKEN'). "
            "Repo-local .praas.toml overrides values set here."
        ),
    )
    ...
    parser.add_argument('command', type=str, help='Command to run', choices=commands, default='review')
...
def run(inargs=None, args=None):
    ...
    # short-circuit the PRAAS_CONFIG_BRANCH env fallback before precedence.
    cli_branch = (getattr(args, "config_branch", None) or "").strip()
    env_branch = (os.environ.get("PRAAS_CONFIG_BRANCH") or "").strip()
    ...
    async def inner():
        if args.issue_url:
            result = await asyncio.create_task(Praas().handle_request(args.issue_url, [command] + args.rest))
        else:
            target = args.pr_url if args.pr_url else "local_diff"
            result = await asyncio.create_task(Praas().handle_request(target, [command] + args.rest))
