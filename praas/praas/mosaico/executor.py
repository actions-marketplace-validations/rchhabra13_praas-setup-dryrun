...
PraasExecutor.execute runs a single non-streaming path: it FIRST installs a
...
the inbound text to a praas command, and completes the Task with the rendered
markdown.
...
from praas.mosaico.dispatch import route_and_run_result
from praas.mosaico.observability import (langfuse_span,
                                            mosaico_log_context,
                                            parse_observability_metadata)


class PraasExecutor(AgentExecutor):
    """Turns a MOSAICO message/send into a praas run and returns a Task."""
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        raise NotImplementedError("cancel is not supported by the praas solution agent")


async def health_check() -> str:
    """LLM-connectivity probe for /health. NO-RETRY: bypasses praas's retry-wrapped
    ...
    try:
        ...
        # Construct the handler purely for its side effect of applying praas's LLM
        # config (api_base/key/callbacks/etc.) onto the litellm module — do NOT call its
        # retry-wrapped chat_completion.
        from praas.algo.ai_handlers.litellm_ai_handler import \
            LiteLLMAIHandler
