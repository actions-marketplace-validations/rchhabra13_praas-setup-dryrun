"""Text router: turn inbound MOSAICO text into a praas command and return the
rendered markdown.
...
def _empty_fallback(verb: str) -> str:
    return f"praas {verb}: no output produced (e.g. no files/changes detected)."


def _error_fallback(verb: str) -> str:
    return f"praas could not complete the {verb} (internal error; see agent logs)."
...
def _ask_needs_context_fallback() -> str:
    ...
    return "praas requires a PR URL or a supplied diff."
...
async def _fetch_public_diff(pr_url: str) -> Optional[str]:
    ...
    headers = {"User-Agent": "praas-mosaico"}
    try:
...
def _pr_fetch_failed_fallback(pr_url: str) -> str:
    return (f"praas could not fetch a public diff for {pr_url} "
...
async def _run_praas(target: str, verb: str) -> "RouteResult":
    """Run a review/improve/describe verb via Praas.handle_request, defensively.
...
async def _run_ask(target: str, question: str) -> "RouteResult":
    ...
    arg-injection trick used by _run_praas cannot apply here. Instead, force
    ...
    from praas.tools.pr_questions import PRQuestions
...
async def _run_on_diff(diff_body: str, verb: str, text: str, title: str, empty_ok: bool = True) -> "RouteResult":
    ...
    return await _run_praas("mosaico://supplied-diff", verb)


async def route_and_run_result(user_text: str) -> "RouteResult":
    """Route inbound text to a praas command and return a RouteResult. Never raises."""
    try:
        text = user_text or ""
