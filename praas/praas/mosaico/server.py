...
# Fully load praas's config first. praas.log imports config_loader, whose
# module-level load_file() lazily triggers Dynaconf's custom_merge_loader, which in turn
# imports praas.log. Importing config_loader as the first praas import here ensures
...
import praas.config_loader  # noqa: F401  (import-order load; do not remove)

from praas.log import LoggingFormat, get_logger, setup_logger
from praas.mosaico.card import build_agent_card
from praas.mosaico.env_bridge import apply_mosaico_env, langfuse_env_present
from praas.mosaico.executor import PraasExecutor, health_check
...
def _configure_runtime() -> None:
    ...
    sink as an import-time side effect runs *before* praas's lazy Dynaconf config has
    finished loading, which trips a circular import (praas.log <-> custom_merge_loader)
...
def start() -> None:
    ...
    app = build_app()
    get_logger().info(f"Starting praas MOSAICO solution agent on {host}:{port}")
