...
from praas.algo.cli_args import CliArgs
...
from praas.tools.pr_add_docs import PRAddDocs
from praas.tools.pr_code_suggestions import PRCodeSuggestions
from praas.tools.pr_config import PRConfig
from praas.tools.pr_description import PRDescription
from praas.tools.pr_generate_labels import PRGenerateLabels
from praas.tools.pr_help_message import PRHelpMessage
from praas.tools.pr_line_questions import PR_LineQuestions
...
from praas.tools.pr_update_changelog import PRUpdateChangelog
...
command2class = {
    ...
    # SECURITY: "/help_docs" is disabled because its repository-cloning path does not
    # validate the clone target.
}
...
class Praas:
    ...
    async def _handle_request(self, pr_url, request, notify=None) -> bool:
        ...
        with get_logger().contextualize(command=action, pr_url=pr_url):
            get_logger().info("praas request handler started", analytics=True)
            if action == "answer":
                if notify:
                    notify()
