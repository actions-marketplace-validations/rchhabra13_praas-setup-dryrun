...
from praas.algo.utils import get_max_tokens
...
class PRSimilarIssue:
    def __init__(self, issue_url: str, ai_handler, args: list = None):
        ...
        index_name = self.index_name = "praas-issues"
        try:
            import pandas as pd
        except ImportError as exc:
            raise ImportError("Please install 'pandas' to use the similar_issue tool") from exc
        self._pandas = pd
        ...
        if get_settings().pr_similar_issue.vectordb == "pinecone":
            ...
            self._pinecone_dataset = Dataset
            self._pinecone_dataset_metadata = DatasetMetadata
            # Read Pinecone credentials from the merged settings.
            try:
    ...
    async def run(self):
        ...
        if get_settings().pr_similar_issue.vectordb == "pinecone":
            pinecone_index = self._pinecone.Index(index_name=self.index_name)
    ...
    def _update_index_with_issues(self, issues_list, repo_name_for_index, upsert=False):
        ...
        df = self._pandas.DataFrame(corpus.model_dump()["documents"])
        get_logger().info('Done')
        ...
        df["values"] = embeds
        meta = self._pinecone_dataset_metadata.empty()
        meta.dense_model.dimension = len(embeds[0])
        ds = self._pinecone_dataset.from_pandas(df, meta)
        get_logger().info('Done')
    ...
    def _update_table_with_issues(self, issues_list, repo_name_for_index, ingest=False):
        ...
        if not ingest:
            ...
            if self.index_name not in self.db.table_names():
                raise RuntimeError(f"Cannot ingest issues: table {self.index_name} does not exist")
...
class Metadata(BaseModel):
    ...
    repo: str
    username: str = Field(default="@praas")
