...
from praas.custom_merge_loader import (MAX_TOML_SIZE_IN_BYTES,
...
# from a repo's .praas.toml. For each section listed here, only the keys in
...
# overridable: it points at the praas host's filesystem, so letting a repo set
...
def _resolve_extra_config_to_file(source):
    ...
    Accepts:
      - http:// or https:// URL: fetched via urllib (with optional auth header
        from PRAAS_EXTRA_CONFIG_AUTH_HEADER, e.g. "PRIVATE-TOKEN: <token>").
    ...
    auth_header = os.environ.get("PRAAS_EXTRA_CONFIG_AUTH_HEADER")
    if auth_header:
        if ":" in auth_header:
            ...
            get_logger().warning(
                "PRAAS_EXTRA_CONFIG_AUTH_HEADER is set but malformed "
...
def _reapply_env_overrides():
    ...
    config file or repo .praas.toml can silently replace a secret supplied
...
def _apply_settings_from_file(path: str, label: str):
    ...
    try:
        dynconf_kwargs = {
            "core_loaders": [],
            "loaders": ["praas.custom_merge_loader"],
            "merge_enabled": True,
        }
        try:
        ...
        # Do NOT log the merged dict: external/repo .praas.toml may contain
...
def apply_repo_settings(pr_url):
    ...
    # extra file. Repo-local .praas.toml is still applied later and overrides
...
def _apply_repo_settings_file(repo_settings_file):
    ...
    validate_file_security(parsed_toml, ".praas.toml")
    ...
    _reapply_env_overrides()
    # Do NOT log the merged dict: repo/global .praas.toml may contain secrets
    ...
    try:
        ...
        for err in config_errors:
            if err:
                ...
                header = f"❌ **praas failed to apply '{config_type}' repo settings**"
                body = (
                    f"{header}\n\nThe configuration file needs to be a valid "
                    "[TOML](https://praas-docs.praas.ai/usage-guide/configuration_options/), please fix it.\n\n"
                )
                body += f"___\n\n**Error message:**\n`{err_message}`\n\n"
                if config_type == "global":
                    # Global content is redacted, so we never render it — skip decoding it entirely.
                    # Global settings live in a `praas-settings` repo scoped per platform
                    # (GitHub organization, GitLab group, or Bitbucket workspace).
                    body += "\n\nThe invalid configuration came from the global `praas-settings` settings repository."
                else:
