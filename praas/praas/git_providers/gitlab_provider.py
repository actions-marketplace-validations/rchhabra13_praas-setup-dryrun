...
class GitLabProvider(GitProvider):
    ...
    # Example: https://gitlab.com/praas/praas.git and branch: t1 -> prefix: "https://gitlab.com/praas/praas/-/blob/t1", suffix: "?ref_type=heads"
    ...
    def get_repo_settings(self):
        ...
        try:
            ...
            contents = self.gl.projects.get(self.id_project).files.get(file_path='.praas.toml', ref=main_branch).decode()
            if contents:
                settings_files.append(("local", contents))
        except GitlabGetError:
            pass  # a missing local .praas.toml is expected
        except Exception as e:
            get_logger().warning(f"Failed to load local .praas.toml file, error: {e}")
        return settings_files if settings_files else ""

    def _get_global_repo_settings(self):
        # Load an org-wide <group>/praas-settings/.praas.toml (GitLab.com groups only).
    ...
    def _fetch_global_repo_settings(self, group):
        try:
            project = self.gl.projects.get(f"{group}/praas-settings")
            return project.files.get(file_path='.praas.toml', ref=project.default_branch).decode()
        except GitlabGetError:
            # A missing praas-settings project/file is an expected fallback -> return "" (cached).
            return ""
    ...
    def _prepare_clone_url_with_token(self, repo_url_to_clone: str) -> str | None:
        ...
        # For example: For repo url: https://gitlab.praas-inc.com/praas/autoscraper.git
        # Then to clone one will issue: 'git clone https://oauth2:<access token>@gitlab.praas-inc.com/praas/autoscraper.git'
