...
from praas.algo.utils import Range, process_description
...
# Only cache reasonably-sized settings blobs; a valid .praas.toml is tiny. This bounds the
...
def get_cached_global_settings(cache_key, fetch_fn):
    """Return the org/group/workspace global .praas.toml via a bounded TTL cache.
...
class GitProvider(ABC):
    ...
    def get_pr_description(self, full: bool = True, split_changes_walkthrough=False) -> str | tuple:
        from praas.algo.utils import clip_tokens
        from praas.config_loader import get_settings
    ...
    def get_user_description(self) -> str:
        ...
        # if the existing description wasn't generated by praas, just return it as-is
        if not self._is_generated_by_praas(description_lowercase):
            get_logger().info(f"Existing description was not generated by praas")
            self.user_description = description
            return description

        # if the existing description was generated by praas, but it doesn't contain a user description,
        # return nothing (empty string) because it means there is no user description
        user_description_header = "### **user description**"
        if user_description_header not in description_lowercase:
            get_logger().info(f"Existing description was generated by praas, but it doesn't contain a user description")
            return ""

        # otherwise, extract the original user description from the existing praas description and return it
    ...
    def _possible_headers(self):
        return ("### **user description**", "### **pr type**", "### **pr description**", "### **pr labels**", "### **type**", "### **description**",
                "### **labels**", "### 🤖 generated by praas")

    def _is_generated_by_praas(self, description_lowercase: str) -> bool:
        possible_headers = self._possible_headers()
