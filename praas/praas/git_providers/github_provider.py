...
class GithubProvider(GitProvider):
    ...
    # Example: https://github.com/the-praas/praas.git and branch: v0.8 -> prefix: "https://github.com/the-praas/praas/blob/v0.8", suffix: ""
    ...
    def _publish_check_run(self, text: str, name: str) -> bool:
        if not getattr(self, 'last_commit_id', None):
            ...
            return False
        conclusion = "neutral"
        check_run_name = f"praas - {name.capitalize()}"
    ...
    def get_repo_settings(self):
        ...
        # settings value doesn't short-circuit the PRAAS_CONFIG_BRANCH fallback.
        ...
        if config_branch:
            ...
            try:
                contents = self.repo_obj.get_contents(".praas.toml", ref=config_branch).decoded_content
                if settings_files:
                ...
                if e.status != 404:
                    raise
                get_logger().debug(
                    f"No .praas.toml on branch '{config_branch}', falling back to default branch")
        try:
            # more logical to take 'praas.toml' from the default branch
            contents = self.repo_obj.get_contents(".praas.toml").decoded_content
            if config_branch and not settings_files:
                return contents
            settings_files.append(("local", contents))
        except GithubException as e:
            # A missing local .praas.toml (404) is expected for most repos; log it quietly to
            # avoid warning noise, and surface only unexpected errors as warnings.
            if e.status == 404:
                get_logger().debug("No local .praas.toml found; using existing settings")
            else:
                get_logger().warning(f"Failed to load .praas.toml file, error: {e}")
        except Exception as e:
            get_logger().warning(f"Failed to load .praas.toml file, error: {e}")
    ...
    def _fetch_global_repo_settings(self, repo_owner):
        try:
            global_settings_repo = self.github_client.get_repo(f"{repo_owner}/praas-settings")
            return global_settings_repo.get_contents(".praas.toml").decoded_content
        except GithubException as e:
            # A missing praas-settings repo/file (404) or lack of access (403) is an expected,
            # stable fallback (skip global settings, continue with local) — return "" so it's cached.
            if e.status in (403, 404):
                get_logger().debug(
                    "No accessible organization global .praas.toml; using local settings only",
        ...
        if self.deployment_type == 'app':
            ...
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/Praas/praas#method-2-run-from-source") from e
    ...
    def _prepare_clone_url_with_token(self, repo_url_to_clone: str) -> str | None:
        ...
        #https://github.com/Praas/praas-pro.git
        #Need to embed inside the github token:
        #https://<token>@github.com/Praas/praas-pro.git
