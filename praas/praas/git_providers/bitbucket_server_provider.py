...
class BitbucketServerProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            content = self.bitbucket_client.get_content_of_file(self.workspace_slug, self.repo_slug, ".praas.toml")
            ...
            # A missing .praas.toml is an expected, optional case (like the other
            ...
            get_logger().info(f"Failed to load .praas.toml file, error: {e}")
            return ""
    ...
    # Bitbucket Server access tokens are sent through the Authorization header.
    def _clone_inner(self, repo_url: str, dest_folder: str, operation_timeout_in_seconds: int=None):
        bearer_token = self.bearer_token
        if not bearer_token:
            # Keep this guard because callers can invoke _clone_inner directly.
